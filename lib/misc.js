...
/**
 * @typedef {Object} RemoveDuplicatesResult
 * @property {Object[]} array - the non duplicates that should be persisted to the database.
 * @property {Object[]} duplicates - duplicate array entries that should not be persisted (or would return an error if they would).
 * @property {Object} latestData - a convenience mapping of [_id] => datum
 */

/**
 * Returns data that should be inserted into the database taking into account existing data.
 *
 * @param {Object[]} existingDBData - Array of data already existing in the database.
 * @param {Object[]} inputData - Array of data that user intends to save to the database.
 * @return {RemoveDuplicatesResult}
 */
function removeDuplicates(existingDBData, inputData) {
  const latestData = {};
  for (const datum of existingDBData) {
    latestData[datum._id] = datum;
  }

  // Duplicate inserted data may exist in the inputData array. This occurs
  // when there is multiple data with the same id / _id. This is not allowed
  // and in the serial code would return a { statusCode: 400,
  // errorCode: 'duplicate', message: 'received a duplicate event'} error to
  // the callback. Since there may be multiple duplicates we instead include
  // an array of duplicates in the returned object.
  const finalArray = [];
  const duplicates = [];
