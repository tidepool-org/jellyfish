...
var userIdsPath = function (env) {
  return {
    hashedPath: `${__dirname}/platform_users/${env}_hashed.json`,
    unhashedPath: `${__dirname}/platform_users/${env}.json`,
  };
};

var hashItem = function (item, salt) {
  if (!item) {
    throw new Error('Missing required userid');
  }
  if (!salt) {
    throw new Error('Missing required salt');
  }
  return crypto.createHmac('sha256', salt).update(item).digest('hex');
};

exports.hashUserId = hashItem;

// Function to encrypt the contents of a JSON file
exports.hashUserIds = function (env, salt) {
  const filePaths = userIdsPath(env);

  if (!fs.existsSync(filePaths.unhashedPath)) {
    throw new Error(`Missing required file ${filePaths.unhashedPath}`);
  }

  let hashedArray = [];
  if (fs.existsSync(filePaths.hashedPath)) {
    const data = fs.readFileSync(filePaths.hashedPath, 'utf8');
    hashedArray = JSON.parse(data);
  }

  const data = fs.readFileSync(filePaths.unhashedPath, 'utf8');
  const jsonArray = JSON.parse(data);

  jsonArray.forEach((userid) => {
    const hashedId = hashItem(userid, salt);
    if (!hashedArray.includes(hashedId)) {
      hashedArray.push(hashItem(userid, salt));
    }
  });

  fs.writeFileSync(filePaths.hashedPath, JSON.stringify(hashedArray));
  console.log(`Hashed user IDs saved as ${filePaths.hashedPath}`);
};
...
exports.validPlatformUserId = function (env, salt, userid) {
  const filePaths = userIdsPath(env);
  if (!fs.existsSync(filePaths.hashedPath)) {
    throw new Error(`Missing required file ${filePaths.hashedPath}`);
  }
  ...
  const data = fs.readFileSync(filePaths.hashedPath, 'utf8');
  const jsonArray = JSON.parse(data);
  const hashedUserId = hashItem(userid, salt);
  return jsonArray.includes(hashedUserId);
};
