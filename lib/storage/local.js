  ...
  return {
    ...
    get: function(fileConfig, privatePair, cb) {
      ...
      if (fileConfig.encryption != 'none') {
        filePath = storage.decrypt(new Buffer.from(filePath, 'hex'), 'aes256', privatePair);
        if (!filePath) {
      ...
      fs.open(filePath, 'r', function(err, fd) {
        ...
        var dataStream = fs.createReadStream(null, {fd: fd});
        if (fileConfig.encryption != 'none') {
          log.info('Applying %s decryption after reading', fileConfig.encryption);
          dataStream = dataStream.pipe(crypto.createDecipher(fileConfig.encryption, storage.calculateEncryptionKey(privatePair)), {end: false});
        }
    ...
    save: function(userId, privatePair, fileName, dataStream, updates, cb) {
      ...
      var baseDirectory = path.resolve(path.join(directory, userId, moment.utc().format()));
