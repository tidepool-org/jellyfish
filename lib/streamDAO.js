      ...
      mongoClient.withCollection('summary', cb, function(coll, cb){
        ...
        coll.findOne(selector, function(err, summary){
          if (err != null) {
            ...
            if (summary?.dates?.outdatedReason === undefined || summary?.dates?.outdatedReason === null) {
              update.$set["dates.outdatedReason"] = [outdatedReasonDataAdded];
            } else {
              update.$addToSet = {"dates.outdatedReason": outdatedReasonDataAdded};
            }
