...
module.exports = function(mongoClient){
  ...
  return {
    ...
    setSummaryOutdated: function(userId, type, cb) {
      /*
      ...
      */
      mongoClient.withCollection('summary', cb, function(coll, cb){
        ...
        var update = { "$set": {"dates.outdatedSince": timestamp} };
        var selector = { "userId": userId, "type": type };

        coll.findOne(selector, opts, function(err, summary){
          if (err != null) {
            cb(err);
          } else if ( !summary || !(summary.dates ? summary.dates.outdatedSince : undefined) ) {
