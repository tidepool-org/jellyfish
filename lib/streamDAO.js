...
module.exports = function(mongoClient){
  ...
  async function updateDatumInternal(session, collection, datum) {
    ...
    clone = misc.ensureInternalId(clone);
  ...
  return {
    ensureInternalId: misc.ensureInternalId,
    generateInternalId: misc.generateInternalId,
    generateExternalId: misc.generateExternalId,
    ...
    /**
     * callback for getDataXYZ array returning functions
     *
     * @callback getDataArrayCallback
     * @param {error} err
     * @param {Object[]} data - array of datum objects.
     */

    /**
     * callback for getDataXYZ single object returning functions
     *
     * @callback getDataObjectCallback
     * @param {error} err
     * @param {Object} datum - returned result - may be null.
     */

    /**
     * returns all data whose _id is within the provided ids array.
     *
     * @param {Array} ids - array of _id's.
     * @param {getDataArrayCallback} cb - results
     */
    getDataIn: function(ids, cb) {
    ...
    /**
     * returns all data within a within the closed interval [startTime, endTime] as well as the latest datum whose time is before startTime.
     *
     * @param {object} datum - a reference datum. All data with the same (type, deviceId, source) as the input datum and within the time range will be returned.
     * @param {date} startTime - return data with time >= startTime (plus one datum before this).
     * @param {date|string} endTime - return data with time <= endTime
     * @param {date|string} endTime - return data with time <= endTime
     * @param {getDataArrayCallback} cb - results
     */
    getDataInTimeRangeAndBefore: function(datum, startTime, endTime, cb) {
      ...
      mongoClient.withCollection('deviceData', cb, (coll, cb) => {
        ...
        find((err, arr) => {
          ...
          const earliestDatum = arr[0] || Object.assign({}, _.pick(datum, ['_groupId', '_active', 'type', 'deviceId', 'source']), {time: startTime});
    ...
    /**
     * returns the matching datum who's .time property is before the input datum's time property.
     *
     * @param {object} datum - a reference datum. Only a datum with the same (type, deviceId, source) as the input datum will be returned.
     * @param {getDataObjectCallback} cb - result
     */
    ...
    insertDatum: function(datum, cb) {
      ...
      datum = misc.ensureInternalId(datum);
