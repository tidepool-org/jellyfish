...
// as part of migration process we attach the platform style _deduplicator using the 'legacy' hash
function setDeduplicator(datum) {
  if (isUploadType(datum)){
    datum._deduplicator = {
      name: 'org.tidepool.deduplicator.device.deactivate.legacy.hash',
      version: '0.0.0',
    };
  } else {
    datum._deduplicator = {
...
module.exports = function(mongoClient){
  ...
  async function updateDatumInternal(session, collection, datum) {
    ...
    previous = setDeduplicator(previous);
