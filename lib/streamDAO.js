...
function filterDatumForMongo(datum) {
  ...
  if (datum) {
    ...
    // it would be better if this could be done right at input, but its simpler this way for now.
    if ('time' in datum && typeof(datum.time) === 'string') {
      datum.time = new Date(datum.time);
    }
    if ('deviceTime' in datum && typeof(datum.deviceTime) === 'string') {
      datum.deviceTime = new Date(datum.deviceTime);
    }
  }
  return datum;
}
