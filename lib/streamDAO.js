...
module.exports = function(mongoClient){
  ...
  return {
    getDatum: function(id, groupId, cb) {
      // From just id and groupId, don't know which collection datum belongs
      // to so try both - note even after migration of dataSets to
      // deviceDataSets collection we would still need to have both
      // collections here as there's no way to tell if the user is querying
      // for a dataSet or "regular" datum.
    ...
    setSummaryOutdated: function(userId, cb) {
      /*
      ...
      */
      mongoClient.withCollection('summary', cb, function(coll, cb){
        ...
        var update = { "$set": {"outdatedSince": timestamp} };
        var selector = { "userId": userId };

        coll.findOne(selector, opts, function(err, summary){
          if (err != null) {
            cb(err);
          } else if ( !summary || !summary.outdatedSince ) {
    ...
    getSummary: function(userId, cb) {
      //This function is currently unused for anything but tests
        mongoClient.withCollection('summary', cb, function(coll, cb){
          coll.findOne({'userId': userId}, cb);
      });
    },
