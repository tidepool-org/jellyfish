...
// as part of migration process we attach the platform style
function addDeduplicator(datum) {
  if (isUploadType(datum)){
    datum._deduplicator = {
...
function filterDatumForMongo(datum) {
  ...
  if (datum) {
    for (
      var entry = datum.payload, i = 0;
      entry && entry.suspended;
      entry = entry.suspended, i++
    ) {
    ...
    if ('deviceId' in datum && datum.deviceId.includes(BadDeviceIDPrefix)) {
      datum.deviceId = datum.deviceId.replace(
        BadDeviceIDPrefix,
        GoodDeviceIDPrefix
      );
...
function isUploadType(datum) {
  return (
    typeof datum?.type === 'string' && datum.type.toLowerCase() === 'upload'
  );
}

module.exports = function (mongoClient) {
  function getCollectionName(datum) {
    return isUploadType(datum) ? 'deviceDataSets' : 'deviceData';
  }
  ...
  async function updateDatumInternal(session, collection, datum) {
    ...
    if (previous == null) {
      throw {
        statusCode: 400,
        message: "Asked to update a datum that doesn't exist",
      };
    }
  ...
  return {
    ...
    getDatum: function (id, groupId, cb) {
      async function get() {
        ...
        const obj = await mongoClient
          .collection('deviceDataSets')
          .findOne(query);
        if (obj) {
          return obj;
        }
    ...
    getDatumBefore: function (datum, cb) {
      mongoClient.withCollection(
        getCollectionName(datum),
        cb,
        function (coll, tempCb) {
    ...
    insertDatum: function (datum, cb) {
        ...
        if (err != null) {
          if (err.code === 11000) {
            return cb({
              statusCode: 400,
              errorCode: 'duplicate',
              message: 'received a duplicate event',
            });
          }
          return cb(err);
        }
      ...
      mongoClient.withCollection(getCollectionName(datum), cb, function (coll) {
    ...
    updateDatum: function (datum, cb) {
      ...
      const updateInTx = async (session) => {
        ...
        firstResult = await updateDatumInternal(
          session,
          collection,
          filteredDatum
        );
    ...
    setSummaryOutdated: function (userId, typ, cb) {
      ...
      mongoClient.withCollection('summary', cb, function (coll, cb) {
        ...
        let update = {
          "$addToSet": {
            "dates.outdatedReason": outdatedReasonDataAdded,
          },
          "$set": {
            "dates.outdatedSince": timestamp,
        ...
        coll.findOne(selector, function (err, summary) {
          if (err != null) {
            ...
            summary?.dates?.outdatedSince === undefined ||
            summary?.dates?.outdatedSince === null
          ) {
