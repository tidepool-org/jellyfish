...
module.exports = function(streamDAO) {
  ...
  return {
    ...
    setSummariesOutdated(userId, data, uploadedCount, cb) {
      if (!userId) {
        return cb();
      }

      const types = summary.getUpdatedSummaryTypes(data, uploadedCount);
      const outdatedReason = summary.getOutdatedReason(data);
      const outdatedBuffer = summary.getOutdatedBuffer(outdatedReason);

      async.eachOfSeries(types, function (typ, key, next) {
        const outdated = { outdatedBuffer, outdatedReason, typ, userId};
        streamDAO.setSummaryOutdated(outdated, function(err) {
          if (err != null) {
            log.error(err, `Problem marking ${typ} summary outdated for user ${userId}.`);
          }
          next();
        });
      }, cb);
