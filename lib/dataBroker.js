...
module.exports = function(streamDAO, workClient) {
  ...
  return {
    ...
    // recalculates every summary of the user. The batch is reported whatever it contains, as data
    // that feeds no summary is still postprocessed. Errors are logged and never fail the upload,
    // matching the platform producers.
    createUploadPostprocessWork(userId, data, cb) {
      if (!userId) {
        return cb();
      }
