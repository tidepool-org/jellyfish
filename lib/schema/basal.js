...
module.exports = function(streamDAO){
  function updatePreviousDuration(dataOrDatum, cb) {
    ...
    // Optimization for slow tandem uploads BACK-2514.
    // Rather than the original way of getting the previous entry one by one
    // for each datum, we find all datums between the min and max time of an input
    // array of data. This way we can check the previous with only one DB call.
    // There is a caveat though, because we have an array of data, the previous of a datum
    // might not exist in the database yet. It might be in the array. So that is what
    // we need to be mindful of. We need to check the results from the database as well
    // as the input array. If the previous datum exists in the input array, use that
    // over the database entry as that indicates the previous datum needs to be updated
    // as well.
    streamDAO.getDataInTimeRangeAndBefore(array, minTime, maxTime, (err, existingData) => {
      if (err != null) {
        return cb(err);
      }
