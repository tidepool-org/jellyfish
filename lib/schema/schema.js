...
exports.makeHandler = function(key, spec) {
  ...
  /*
    ...
    dontPostTransform - if true and there is a postTransformIfNotGrouped
    handler then don't apply the postTransform and use an identify function
    instead. This is because handler.postTransformIfNotGrouped is meant to
    only be called by itself if the datum is not part of a "group" or array
    of datums. This allows us to perform an optimization for BACK-2514 by
    transforming all data that requires database access in one step rather
    than an individual postTransformIfNotGrouped call for each datum.
   */
  function decepticon(datum, dontPostTransform, cb) {
    ...
    try {
      transformer(datum, function(err, newDatum){
        ...
        let postTransform = spec.postTransformIfNotGrouped;
        // If postTransformIfNotGrouped doesn't exist or we shouldn't call it
        // (currently we only don't call it for type == 'basal' and the input
        // datum being an array), then just use a "blank" function that
        // returns the datum as is.
        if (typeof postTransform !== 'function' || dontPostTransform) {
          postTransform = function(datum, cb) { return cb(null, datum); };
        }
        postTransform(newDatum, (err, newestDatum) => {
          if (err != null) {
            return cb(err);
          }
          var withId = attachId(newestDatum);
  ...
  // can be called as retVal(datum, cb)
  // or retVal(datum, opts, cb)
  var retVal = function (datum, opts, cb) {
    // The previous function signature for retVal was `function (datum, cb)`
    // so in order to support that check if opts is actually the cb.
    ...
    decepticon(datum, !!opts.dontPostTransform, cb);
  };
...
// postTransformGroup is an optional function for handling multiple datum
// (data) at a time as an array instead of 1 by 1 as an optimization for
// BACK-2514. It is applied as a final step to the all input data after they
// are transformed by a subhandler's regular transform function. This is
// currently only used for types that used to read a previous datum from the
// database one by one for each datum. Instead the postTransformGroup should
// read all previous datums in a single DB call for improved performance.
exports.makeSubHandler = function(value, key, subHandlersArray, postTransformGroup=null) {
  var subHandlers = {};
  ...
  var retVal = function (dataOrDatum, cb) {
    const originalArray = Array.isArray(dataOrDatum) ? dataOrDatum : [dataOrDatum];

    // If there is a postTransformGroup function, that means it is intended to
    // be done on the final array as an optimization step instead of the the
    // handler's normal postTransformIfNotGrouped function
    const dontPostTransform = originalArray.length > 1 && typeof postTransformGroup === 'function';
    const shouldGroupTransformAfter = dontPostTransform;

    async.map(originalArray, (datum, asyncCb) => {
      var type = datum[key];
      ...
      const opts = {
        dontPostTransform: dontPostTransform,
      };
      subHandler(datum, opts, asyncCb);
    }, (err, newArray) => {
      if (err) {
        return cb(err);
      }

      // newArray is the concatenation of a subhandler running for each
      // element in dataOrDatum and each subhandler may return an array
      // itself, so this flattens everything into a single array.
      let finalArray = [];
      ...
      // The previous withId code in makeHandler would return an object if the
      // final result was just a one element array , otherwise it would
      // return the array so this just mimicks that.
      const singularize = (err, results) => {
        if (err) {
          return cb(err);
        }
      ...
      if (shouldGroupTransformAfter && typeof postTransformGroup === 'function') {
        postTransformGroup(finalArray, singularize);
      } else {
        singularize(null, finalArray);
      }
    });
  };

  retVal.postTransformGroup = postTransformGroup;
  retVal.key = value;
