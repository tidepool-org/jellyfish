...
exports.makeHandler = function(key, spec) {
   ...
   */
  function decepticon(datum, shouldNotPostTransform, cb) {
    ...
    try {
      transformer(datum, function(err, newDatum){
        ...
        let postTransform = spec.postTransform;
        // If postTransform doesn't exist or we shouldn't call it(currently we
        // only don't call it for type == 'basal' and the input datum being
        // an array), then just use a "blank" function that returns the datum
        // as is.
        if (typeof postTransform !== 'function' || shouldNotPostTransform) {
  ...
  var retVal = function (datum, opts, cb) {
    ...
    decepticon(datum, !!opts.shouldNotPostTransform, cb);
  };

  retVal.key = key;
  retVal.supportsInsertParallel = false;

  return retVal;
};

// groupTransform is an optional function for handling multiple datum(data) at
// a time as an array instead of 1 by 1. It is applied as a final step to the
// all the transformed data. This is currently only used for basals as an
// optimization for BACK-2514.
exports.makeSubHandler = function(value, key, subHandlersArray, groupTransform=null) {
  var subHandlers = {};
  ...
  var retVal = function (dataOrDatum, cb) {
    ...
    const shouldNotPostTransform = originalArray.length > 1;
    const shouldGroupTransformAfter = shouldNotPostTransform;
    ...
    async.map(originalArray, (datum, asyncCb) => {
      ...
      const opts = {
        shouldNotPostTransform: shouldNotPostTransform,
      };
      ...
      if (shouldGroupTransformAfter && typeof groupTransform === 'function') {
        groupTransform(finalArray, singularize);
      } else {
  ...
  retVal.groupTransform = groupTransform;
  retVal.key = value;
  retVal.supportsInsertParallel = !!groupTransform;
