...
exports.makeSubHandler = function(value, key, subHandlersArray, groupTransform=null) {
  ...
  var retVal = function (dataOrDatum, cb) {
    ...
    async.map(array, (datum, asyncCb) => {
      ...
      const modifiedSubHandler = subHandler.bind({shouldNotPostTransform: array.length > 1});
      ...
      if (array.length > 1 && typeof groupTransform === 'function') {
        groupTransform(finalArray, cb);
      } else {
        cb(err, finalArray);
      }
    });
