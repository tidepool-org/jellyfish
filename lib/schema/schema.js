...
exports.makeHandler = function(key, spec) {
  ...
  var retVal = function (datum, cb) {
    ...
    decepticon(datum, !!(this||{}).shouldNotPostTransform, cb);
  };
...
exports.makeSubHandler = function(value, key, subHandlersArray, groupTransform=null) {
  ...
  var retVal = function (dataOrDatum, cb) {
    ...
    async.map(originalArray, (datum, asyncCb) => {
      ...
      const modifiedSubHandler = subHandler.bind({shouldNotPostTransform: originalArray.length > 1});
      modifiedSubHandler(datum, asyncCb);
    }, (err, newArray) => {
      if (err) {
        return cb(err);
      }
      // Each object within array may itself return an array so flatten.
      let finalArray = [];
      ...
      // The previous withId code in makeHandler would return an object
      // if the final result was just one element, otherwise it would return
      // the array so this just mimicks that.
      ...
      if (originalArray.length > 1 && typeof groupTransform === 'function') {
