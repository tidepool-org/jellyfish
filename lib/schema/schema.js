...
exports.generateHash = function(datum) {
  ...
  var idFields = idFieldMap[datum.type];
  if (!idFields){
    return '';
  }

  var vals = new Array(idFields.length);
  for (var i = 0; i < idFields.length; ++i) {
    vals[i] = datum[idFields[i]];
    if (vals[i] == null) {
      throw except.IAE('Can\'t generate hash, field[%s] didn\'t exist on datum of type[%s]', idFields[i], datum.type);
    }
  }
