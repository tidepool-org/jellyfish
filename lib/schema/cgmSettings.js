...
module.exports = schema.makeHandler('cgmSettings', {
  ...
  transform: function(datum, cb) {
    ...
    if (datum.units === 'mg/dL') {
      datum.lowAlerts.level = schema.convertMgToMmolPrecision(datum.lowAlerts.level);
      datum.highAlerts.level = schema.convertMgToMmolPrecision(datum.highAlerts.level);
      datum.rateOfChangeAlerts.fallRate.rate = schema.convertMgToMmolPrecision(datum.rateOfChangeAlerts.fallRate.rate);
      datum.rateOfChangeAlerts.riseRate.rate = schema.convertMgToMmolPrecision(datum.rateOfChangeAlerts.riseRate.rate);
      datum.units = 'mmol/L';
    }
