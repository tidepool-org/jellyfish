...
module.exports = schema.makeHandler('upload', {
  ...
  transform: function(datum, cb) {
    ...
    var MINIMUM_UPLOADER_VERSION='0.200.0';
    ...
    if ( versionStr.indexOf('tidepool-uploader') !== -1 ) {
      ...
      if ( schema.isValidVersion(versionNum, MINIMUM_UPLOADER_VERSION)) {
        console.log('Upload Item: version check passed');
        return cb(null,datum);
      }
      console.log('Upload Item: version check FAILED expected [',MINIMUM_UPLOADER_VERSION,'] got [', versionNum,']');
      return cb({ statusCode: 400, message: { text: 'The minimum supported version is ['+MINIMUM_UPLOADER_VERSION+']. ['+datum.version+'] is no longer supported.', code: 'outdatedVersion', errorField: 'version'}});
    }
    return cb(null,datum);
  }
});