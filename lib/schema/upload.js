...
module.exports = schema.makeHandler('upload', {
  ...
  transform: function(datum, cb) {
    ...
    if ( versionStr.indexOf('tidepool-uploader') !== -1 ) {
      ...
      return cb({ statusCode: 400, text : 'The minimum supported version is ['+MINIMUM_UPLOADER_VERSION+']. Version ['+datum.version+'] is no longer supported.', code: 'outdatedVersion', errorField: 'version'});
    }
    return cb(null,datum);
  }
});