      ...
      async.waterfall(
        [
          ...
          function(userId, groupId, cb) {
            ...
            // "group" similar basals together into a single array so they can
            //  be processed all at once. This is an optimization for
            //  BACK-2514 This is because in the original code, the data is
            //  inserted one by one. And each basal may check and annotate
            //  the previous basal. If we were to just naively insert in
            //  parallel, some of the previous basals might be ones that
            //  aren't yet in the database but are ones that are actually
            //  about to be inserted. By grouping these together the logic to
            //  fetch the previous basal can be done on the application side
            //  and all "grouped" basals can be inserted in parallel. All
            //  other types don't have a dependency on a previous datum so
            //  they can be inserted in parallel w/o this grouping.
            const basalGroups = {};
            ...
            for (const datum of array) {
              ...
              // group all in same even if deliveryType (aka the subHandler key) is
              // different because a basal may refer to a previous basal that is of
              // a different deliveryType.
              const groupingKey = `${datum._groupId}_${datum.type}_${datum.deviceId}_${datum.source}`;

              if (!basalGroups[groupingKey]) {
                basalGroups[groupingKey] = [];
              }
              basalGroups[groupingKey].push(datum);
            }
            groups = groups.concat(Object.values(basalGroups));
