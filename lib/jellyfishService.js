...
function createServer(serverConfig, mongoClient, seagullClient, userApiClient, gatekeeperClient){
  ...
  var schemas = require('./schema')(streamDAO);
  ...
  */
  app.post(
    ...
    function(request, response) {
      ...
      var count = 0;
      var duplicates = [];
      var updatesSummary = {cgm: false, bgm: true};
        ...
        [
          function(cb) {
            ...
            gatekeeperClient.userInGroup(userid, request.params.groupId, function(err, perms) {
              ...
              console.log(`perms duration ${Date.now() - now}`);
          ...
          function(userId, groupId, cb) {
            ...
            // "group" similar types together (that support it) into a single
            //  array so they can be processed all at once. This is an
            //  optimization for BACK-2514 This is because in the original
            //  code, the data is inserted one by one. And each basal may
            //  check and annotate the previous basal. If we were to just
            //  naively insert in parallel, some of the previous basals might
            //  be ones that aren't yet in the database but are ones that are
            //  actually about to be inserted. By grouping these together the
            //  logic to fetch the previous basal can be done on the
            //  application side and all "grouped" basals can be inserted in
            //  parallel. All other types don't have a dependency on a
            //  previous datum so they can be inserted in parallel w/o this
            //  grouping.
            const parallelGroups = {};
            let groups = []; // Array of either a single datum or an array of data.

            for (const datum of array) {
              if (schemas.getGroupableDatumTypes().indexOf(datum.type) === -1) {
                groups.push(datum);
                continue;
              }
              const groupingKey = `${datum._groupId}_${datum.type}`;
              if (!parallelGroups[groupingKey]) {
                parallelGroups[groupingKey] = [];
              }
              parallelGroups[groupingKey].push(datum);
            }
            groups = groups.concat(Object.values(parallelGroups));

            const now = Date.now();
            async.mapLimit(
              groups,
              50,
              function(dataOrDatum, asyncCb) {
                dataBroker.addData(dataOrDatum, updatedSummary, function(err){
                  if (err != null) {
              ...
              (err) => {
                console.log(`addData duration ${Date.now() - now} for ${groups.length} groups`);
                cb(err);
              }
            );
          }
