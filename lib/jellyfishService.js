  ...
  app.post(
    ...
    function(request, response) {
      ...
      async.waterfall(
            ...
            async.mapSeries(
              ...
              function(obj, cb) {
                ...
                dataBroker.addDatum(obj, function(err, datumUpdatesSummary){
                  ...
                  if (err == null) {
                    log.info('datumUpdatesSummary 2 for user[%s] =%s ',userid, datumUpdatesSummary);
                    log.info('updatesSummary 2 for user[%s] =%s ',userid, updatesSummary);
                    if (datumUpdatesSummary == true) {
                        updatesSummary = true;
                    }
                  }
                  ++count;
        ...
        function(err) {
          log.info('updatesSummary 1 for user[%s] =%s ',userid, updatesSummary);
          if (updatesSummary == true) {
            streamDAO.setSummaryOutdated(storedUserId, function(err){
              if (err != null) {
                log.warn(err, 'Problem marking summary outdated for user[%s].', userid);
              }
            });
          }
          if (err != null) {
