  ...
  app.post(
    ...
    function(request, response) {
      ...
      async.waterfall(
          ...
          function(userId, groupId, cb) {
            ...
            async.mapSeries(
              ...
              function(obj, cb) {
                ...
                dataBroker.addDatum(obj, function(err, datumUpdatesSummary){
                  if (err != null) {
                  ...
                  // error will be null here if the error was non-critical
                  if (err == null) {
                    if (datumUpdatesSummary.cgm === true) {
                        updatesSummary.cgm = true;
                    }
                    if (datumUpdatesSummary.bgm === true) {
        ...
        function(err) {
          if (updatesSummary.cgm === true) {
            streamDAO.setSummaryOutdated(storedUserId, 'cgm', function (err) {
              if (err != null) {
                log.warn(err, 'Problem marking cgm summary outdated for user[%s].', userid);
              }
            });
          }

          if (updatesSummary.bgm === true) {
            streamDAO.setSummaryOutdated(storedUserId, 'bgm', function(err){
              if (err != null) {
                log.warn(err, 'Problem marking bgm summary outdated for user[%s].', userid);
              }
            });
          }
