...
function createServer(serverConfig, mongoClient, seagullClient, userApiClient, gatekeeperClient){
  ...
  app.get(
    ...
    checkToken,
    function(request, response) {
      getTaskAndAuthorizeUserForTask(request.params.id, request._tokendata.userid, function(err, task) {
        if (err) {
      ...
      async.waterfall(
        ...
        function(err) {
          if (err != null) {
            ...
            } else if (err.type != null && err.type === 'RuntimeException') {
              response.status(500).send({statusCode: 500, message: 'Error processing uploaded data'});
            } else {
