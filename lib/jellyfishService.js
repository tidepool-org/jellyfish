  ...
  var serviceManager = {
    ...
    startService: function (app, servicePort, cb) {
      ...
      if (envConfig.httpPort != null) {
        ...
        this.theServer = https
          .createServer(envConfig.httpsConfig, app)
          .listen(servicePort, cb);
      }
      if (this.theServer) {
...
function createServer(
  ...
  app.get('/info/?:userId?', function (request, response) {
    log.info('Handling versions request');

    var uploaderDestination = 'jellyfish';
    if (request.params.userId) {
      log.info('using user id to detrimine upload destination');
      if (serverConfig.uploader.platformUsers.includes(request.params.userId)) {
        uploaderDestination = 'platform';
        log.info(`upload to ${uploaderDestination}`);
      }
    }
  ...
  */
  app.post('/data/?:groupId?', checkToken, function (request, response) {
    ...
    if (typeof array !== 'object') {
      return response
        .status(400)
        .send(util.format('Expected an object body, got[%s]', typeof array));
    }
    ...
    async.waterfall(
      ...
      function (err) {
        dataBroker.setSummariesOutdated(
          datasetUserId,
          array,
          count,
          function () {
            if (err != null) {
              if (err.statusCode != null) {
                ...
                var groupMessage = request.params.groupId
                  ? 'To group[' + request.params.groupId + ']'
                  : '';
                log.warn(
                  err,
                  'Problem uploading for user[%s]. %s',
                  userid,
                  groupMessage
                );
                response.status(500);
