...
module.exports = function(config) {
  ...
  function ingest (task) {
    ...
    childProcess.fork('./lib/children/ingest.js', [task._id]);
    console.log("INGESTING?");
  }

  function upload (payload, callback) {
    // Create new empty sync task
    db.syncTasks.save({}, function(err, syncTask) {
      payload.syncTaskId = syncTask._id;
      syncTask.url = '/v1/synctasks/' + syncTask._id;
      // Launch syncing in a separate child process
      childProcess.fork('./lib/children/sync.js', [JSON.stringify(payload)]);

      // Answer request immediately with sync task id
      // so client can keep track of syncing progress
      callback(null, syncTask);
    })
  }
